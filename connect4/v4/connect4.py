...
class Connect4Game():
    ...
    def getMove(self, model, exploit=1):
        ...
        saArray = []    # Array of all possible state-action pairs as flattened network input arrays
        for i in validMoves:
            saArray.append(self.getInput(i))
        saArray = np.array(saArray)
        output = model.predict(saArray)
        predictions = output[:,:1]
        winMoves = np.take(output,1,1)
    ...
    def getMaxQValue(self, model, sPrime):
        ...
        predictions = model.predict(sPrime) # Predict outputs for each sPrime
        predictions = np.take(predictions, 0, 1) # Get first element (q value) for each move
        moveArg = predictions.argmax() # Get index of largest rated q value
        qValue = predictions[moveArg] # Get largest rated q value
        return qValue
