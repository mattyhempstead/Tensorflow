...
MEMORY_MAX_SIZE = 100000     # Approximately 10 samples per game (~1GB per 100000?)
EXPLORE_DECAY = 20
...
while True:
    ...
    for i in range(count):
        ...
        gameResults = playTrainingGames(
            ...
            opponent,
            discountFactor = 0.9, 
            exploit = trainCount / (trainCount + EXPLORE_DECAY)
            # exploit = 1 - math.exp(-trainCount/100)
            # exploit = 0.9
        )
        ...
        for newMemory in zip(gameResults["states"], gameResults["actions"], gameResults["sPrimes"]):
        ...
        states = [memory[0] for memory in memoryStates]
        actions = [memory[1] for memory in memoryStates]
        sPrimes = [memory[2] for memory in memoryStates]
        ...
        agentTargets = model.predict(agentInputs)
        ...
        # Move prediction direction 
        # True if move was predicted better than it actually was
        # False if move was predicted as being worse than it actually was
        movePredictionDirection = []

        for agentTarget, action, qValue in zip(agentTargets, actions, qValues):
            movePredictionDirection.append(agentTarget[action] >= qValue)
            agentTarget[action] = qValue
        # agentTargets = np.array(agentTargets)

        # for agentTarget in agentTargets:
        #     print(agentTarget)
        # input()

        print("Move Directions: {}/{}".format(sum(movePredictionDirection), len(movePredictionDirection)))
